...
PasswordRepository::PasswordRepository() { initDatabase(); }
PasswordRepository::~PasswordRepository() { if (m_db.isOpen()) m_db.close(); }

void PasswordRepository::initDatabase()
{
    m_db = QSqlDatabase::addDatabase("QSQLITE");
    m_db.setDatabaseName("passwords19.db");
...
{
    QSqlQuery query;
    query.prepare("INSERT INTO credentials (title, login, secret, site, category, updated_at) VALUES (?, ?, ?, ?, ?, ?)");
